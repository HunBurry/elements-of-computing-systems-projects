...
class TokType:
    KEYWORD, SYMBOL, IDENTIFIER, NUMBER, STRING = range(5)

def tokType(n):
    return ["keyword", "symbol", "identifier", "integerConstant", "stringConstant"][n]
...
class Token:
    ...
    def __str__(self):
        if self.val == "<":
            return "<%s>%s</%s>" % (tokType(self.type), "&lt;",  tokType(self.type))
        if self.val == ">":
            return "<%s>%s</%s>" % (tokType(self.type), "&gt;",  tokType(self.type))
        if self.val == "&":
            return "<%s>%s</%s>" % (tokType(self.type), "&amp;",  tokType(self.type))
...
class Tokenizer:
    ...
    def fromStart(self, c):
        if isDigit(c): 
            ...
            self.collectGoto(c, TState.COMMENT)
        elif isQuote(c): 
    ...
    def processIdent(self):
        ...
        if self.isKeyword(tok):
            ...
            return Token(TokType.IDENTIFIER, tok)

    def isKeyword(self, tok):
        return tok in ["void", "method", "char", "field", "static", "this", "constructor", "boolean", "class", "while", "function", "var", "let", "do", "return", "null", "int", "if", "else", "true", "false"]
